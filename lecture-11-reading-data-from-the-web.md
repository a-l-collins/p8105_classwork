...
This extracts 15 tables. If we want to only extract one table, such as
the first, we can do:
...
Accessing NYC’s open data via API, instead of downloading them from the
website, so that the data will auto update as the API updates:
...
Can also import data from data.gov as a CSV or JSON (CSV in this
example):
...
By default, the CDC API limits data to the first 1000 rows. We changed
this via the `query = list` command, which was found by poking around
the API website.

### Be Reasonable

Every call is actually reaching out to the website and pulling
information. If every time you knit a doc, it’s pulling a significant
amount of data, it will be noticed. There are also developers who limit
the number of database entries that can be accessed in a single request–
in those cases, you’ll need to iterate thru and combine the end results
